...
- (void)show:(UIImageView *)view
{
    if (drawFlag) {
        ...
        for(int y = 0; y < 32; ++y){
            for(int x = 0; x < 64; ++x){
                if(gfx[(y * 64) + x] == 0){
                    screenData[y][x][0] = 0x54;
                    screenData[y][x][1] = 0x8B;
                    screenData[y][x][2] = 0x54;	// Disabled
                }else{
                    screenData[y][x][0] = 0xFF;
                    screenData[y][x][1] = 0xFF;
                    screenData[y][x][2] = 0xFF;  // Enabled
                }
                screenData[y][x][3] = 255;
            }
        }
        
        int bitmapInfo = kCGBitmapByteOrderDefault | kCGImageAlphaPremultipliedLast;
        CGContextRef ctx = CGBitmapContextCreate(screenData, 64, 32, 8, 64 * 4, CGColorSpaceCreateDeviceRGB(), bitmapInfo);
        CGImageRef imageRef = CGBitmapContextCreateImage(ctx);
        view.layer.contents = (__bridge id _Nullable)(imageRef);
